...
전체 바닥은 46 × 24m이며, 통로 중심이 좌우 최대 2.2m 이동하는 S자 시장길이다.
가게·장애물·쇼핑 정차점도 굽은 통로에 맞췄고, 좌판 사이 병목 폭은 2.1m다.
횡단보도 진입부는 단차 없이 비웠다. 양쪽 끝에 빨간색 서 있는 사람과 초록색 걷는 사람 모양의 보행신호등을 설치했다.
...
| 시간 | 동작 |
|---|---|
| 0–15초 | 빨간 사람 점등. 오토바이 2대가 양쪽 차로를 약 0.99m/s로 이동 |
| 15–17초 | 빨간 사람 유지. 오토바이 대기 |
| 17–24초 | 초록 걷는 사람 점등. 보행자 2명이 반대 방향으로 약 0.62m/s로 횡단 |
| 24–27초 | 초록 사람 점멸: 0.5초 켜짐 / 0.5초 꺼짐. 보행자는 횡단 완료 |
| 27–32초 | 빨간 사람 점등. 대기 후 다음 주기에 보행자가 반대편으로 복귀 |

모델 위치를 갱신하는 스크립트 방식이다. 바퀴 동역학·보행 관절 애니메이션,
휠체어를 감지한 양보·회피는 포함하지 않는다.
보행신호는 이 시뮬레이션의 교통 주기에 맞춘 시각적 신호이며, 교통 제어 규격 인증을 의미하지 않는다.
사람 아이콘은 발광 재질의 SDF 형상이다. 꺼진 아이콘은 충돌 없이 지하로 이동시켜 숨기며
신호등 기둥·검은 렌즈·하우징은 고정된다. `moving_traffic:=false`이면 초기 빨간불로 유지된다.
