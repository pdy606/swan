...
│  situation_c_node /swan/detections → /driving_situation="C" │  ← C: 회피
│  signal_node    /yolo(신호등) → /driving_situation="B"    │  ← B: 의미기반(빨강=정지)
└─────────────────────────────────────────────────────────┘
              A=정상 / B=신호(의미) / C=회피

기존 3노드 구성을 선택해서 실행한다. swan_bringup의 기본 situation_node와 동시에 실행하지 않는다.
...
def generate_launch_description():
    return LaunchDescription([
        ...
        Node(package='swan_pipeline', executable='fusion_node', output='screen',
             parameters=[ST]),
        # C: 회피 판단
        Node(package='swan_pipeline', executable='situation_c_node', output='screen',
