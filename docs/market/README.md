...
## v5 — 굽은 시장길·사람 모양 보행신호 (2026-09-24)

통로 중심을 좌우 최대 2.2m 굽히고 가게·장애물·정차점을 재배치했다.
횡단보도 양쪽에 빨간 서 있는 사람 / 초록 걷는 사람 신호등을 추가했다.
초록불은 17–24초 점등, 24–27초에 1Hz 점멸하며 그 외 시간은 빨간불이다.

오프라인 테스트 16개로 기본 실행 설정, 중복 모델, 신호 전환 경계·반복 주기,
오토바이 이동 중 초록불 금지, 아이콘 충돌 형상 미포함을 확인했다.
정적 경로 검사에서 반경 0.75m 검사 원을 사용했으며 최소 잔여 여유는 약 0.171m다.
쇼핑 정차점 3곳도 정적 장애물과 겹치지 않는다. 휠체어 제외 교통 모델 2주기 형상 검사도 통과했다.
UTM이 꺼져 있어 v5의 실제 렌더링·점멸·주행·YOLO 인식은 아직 실행 검증하지 않았다.

`tools/inspect_market.py`는 `preview/market_plan.png`, `market_isometric.png`,
`pedestrian_signals.png`와 검사 JSON을 생성한다. 이 이미지는 SDF 형상 기반 미리보기이며
Gazebo 캡처가 아니다. 아래 v4 영상과 수치는 이전 배치의 기록이다.
...
이는 공통 런처 개편 전 실행 결과다. 월드 선택 런처 개편 및 팀 원본 휠체어 모델로 정렬한 현재 조합은 VM이 꺼져 있어 실제 Gazebo 재실행 미검증 상태다.
당시 오프라인 테스트 13개로 월드 선택·로봇 중복 생성 방지·브리지 설정 등을 검사했다.
